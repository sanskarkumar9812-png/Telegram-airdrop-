...
def get_chat_id(chat):
    """Convert chat identifier to numeric ID for comparison."""
    try:
        return int(chat)
    except (ValueError, TypeError):
        return chat  # username, will compare as string
...
def extract_bot_username(url: str):
    """Extract bot username from a t.me URL like https://t.me/gifting"""
    try:
    ...
    for row in message.buttons:
        for button in row:
            ...
            if url:
                ...
                if start_param:
                    # Has start param — fire directly via API (fastest)
                    await fire_start_claim(start_param, context)
                else:
                    # No start param — click the button directly
                    log.info("[%s] No start param, clicking button directly...", context)
                    try:
                        await button.click()
                        log.info("[%s] Button clicked ✅", context)
                    except Exception as e:
                        log.info("[%s] Button click failed: %s", context, e)
                return

            if data is not None:
                try:
                    await button.click()
                    log.info("[%s] Clicked callback button ✅", context)
                except FloodWaitError as e:
                    log.warning("Flood wait %ss while clicking", e.seconds)
                except Exception as e:
                    log.info("[%s] Click failed: %s", context, e)
                return


@client.on(events.NewMessage())
async def on_new_message(event):
    # Filter manually to avoid entity resolution errors with private groups
    if event.chat_id not in [get_chat_id(c) for c in TARGET_CHATS]:
        return
    msg = event.message
...
async def main():
    ...
    for chat in TARGET_CHATS:
        try:
            ...
            resolved_chats.append(chat)
            log.info("Resolved chat: %s", chat)
        except Exception as e:
            log.warning("Could not resolve %s: %s — using ID directly", chat, e)
            resolved_chats.append(chat)
    gifting_entity = await client.get_entity(GIFTING_BOT)

    log.info("Watching %s for Stars Airdrops via @%s (min %d stars each) ...",
             ", ".join(str(c) for c in TARGET_CHATS), GIFTING_BOT, MIN_STARS_EACH)
