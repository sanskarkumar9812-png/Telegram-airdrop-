...
async def on_pasha_bot_dm(event):
    ...
    # ── If bot has buttons, process them ──
    if msg.buttons:
        has_join_buttons = False
        ...
        for row in msg.buttons:
            for button in row:
                ...
                if any(w in btn_text for w in [
                    "проверить", "verify", "check", "✅", "подтвердить", "готово"
                ]):
                    has_verify_button = True
                    verify_button = button
                    continue

                # Join any channel URL buttons (not verify buttons)
                if url and "t.me/" in url.lower():
                    ...
                    if m:
                        ...
                        if username.lower() not in (PASHA_BOT.lower(), "start", "share"):
                            try:
                                ...
                                has_join_buttons = True
                            except Exception as e:
                                log.warning("[pasha] Could not join @%s: %s", username, e)

        # Click verify button if found
        if verify_button:
            try:
                log.info("[pasha] Clicking verify: [%s]", verify_button.text)
    ...
    # ── Bot asking for password explicitly ──
    if "пароль" in text_low or "введите" in text_low or \
       ("password" in text_low and ("enter" in text_low or "protected" in text_low)):
        ...
        if latest_password:
            ...
            log.info("[pasha] Bot waiting for password...")
            bot_waiting_for_password = True
        return

    # ── Bot sent a message with no buttons and no password prompt ──
    # This could be after verify succeeded — try sending password now
    if not msg.buttons and latest_password and not bot_waiting_for_password:
        # Only if message doesn't look like a promo/shop message
        skip_words = ["магазин", "баланс", "звёзд", "подарок", "бесплатно",
                      "уже выдано", "млн", "000+", "приглашай"]
        if not any(w in text_low for w in skip_words):
            log.info("[pasha] No buttons, trying password: [%s]", latest_password)
            await client.send_message(pasha_entity, latest_password)
            return
    ...
    elif any(w in text_low for w in ["получил", "успешно", "success", "received", "подарок вручён"]):
