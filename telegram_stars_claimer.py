...
async def on_password_reveal(event):
    ...
    # Skip placeholder values meaning "password is set but not revealed yet"
...
async def on_pasha_bot_dm(event):
    ...
    log.info("[pasha] Bot said: %s", text if text else "(no text)")
    ...
    if msg.buttons and any(w in text_low for w in [
        "subscribe", "join", "channel", "check", "verify",
        "подпишитесь", "подписаться", "каналы", "канал",
        "использования", "проверить", "подписк"
    ]):
    ...
    if ("password" in text_low and ("enter" in text_low or "protected" in text_low)) or \
       ("пароль" in text_low) or ("введите" in text_low):
        if latest_password:
