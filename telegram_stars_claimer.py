...
# This script must NEVER post, reply, or send anything into the group chat
# (TARGET_CHAT). It only ever reads from that chat. The only outgoing
# action permitted anywhere in this file is a private, silent backend call
# directed at the bot itself (StartBotRequest / message.click), which is
# exactly what tapping the claim button does manually - nothing visible
# appears in the group, and no message is posted on your behalf there.
#
# This wrapper makes that a hard guarantee: if any code anywhere in this
# file ever tries to call send_message/send_file targeting TARGET_CHAT,
# it will raise instead of silently sending.
...
async def _guarded_send_message(self, entity, *args, **kwargs):
    ...
    if target_username == TARGET_CHAT.lower():
        raise RuntimeError(
