...
GIFT_CHANNEL = os.environ.get("GIFT_CHANNEL", "PaulGifting")  # channel where gift cards are posted
...
async def on_gift_card_message(event):
    ...
    msg = event.message
    text = msg.text or ""
    if not any(k in text.upper() for k in ["GIFT CARD", "GIFT CHEQUE", "GIFT CHECK"]):
        return
    if not msg.buttons:
...
async def on_password_reveal(event):
    """Password appears in channel → store it. Bot will ask for it and we send it then."""
    msg = event.message
    text = msg.text or ""
    ...
    global latest_password
    latest_password = pw_match.group(1).rstrip(".,;!?)\"'")
...
@client.on(events.NewMessage(from_users=PASHA_BOT))
async def on_pasha_bot_dm(event):
    """
    Handle @PashaGiftsBot's DM responses:
    1. If it says join channels → actually JOIN them, then click Verify
    2. If it asks for password → send it instantly
    ...
    log.info("[pasha] Bot said: %s", text[:120] if text else "(no text)")

    # ── Step 1: Bot wants us to join channels ──
    # It mentions channel usernames or has join buttons in its message.
    # We actually join each channel, then click Verify.
    if msg.buttons and ("subscribe" in text_low or "join" in text_low or "channel" in text_low):

        # Find all t.me channel links mentioned in the message text
        # e.g. "subscribe to @notgambling" or "t.me/notgambling"
        channel_mentions = re.findall(r"(?:t\.me/|@)([a-zA-Z0-9_]{4,})", text)

        for username in channel_mentions:
            # Skip the bot itself and known non-channel words
            if username.lower() in (PASHA_BOT.lower(), "pashagifsbot", "start"):
                continue
            try:
                log.info("[pasha] Joining channel @%s ...", username)
        ...
        # Also check the URL buttons — they may point to channels to join
        for row in msg.buttons:
            for button in row:
                url = getattr(button, "url", None)
                if url and "t.me/" in url.lower():
                    # Extract username from URL like https://t.me/notgambling
                    m = re.search(r"t\.me/([a-zA-Z0-9_]+)", url, re.IGNORECASE)
                    if m:
                        username = m.group(1)
                        if username.lower() not in (PASHA_BOT.lower(), "start"):
                            try:
                                log.info("[pasha] Joining channel from button @%s ...", username)
                                entity = await client.get_entity(username)
                                await client(JoinChannelRequest(entity))
                                log.info("[pasha] Joined @%s", username)
                            except Exception as e:
        ...
        # Now click the Verify / Check subscription button
        for row in msg.buttons:
            for button in row:
                btn_text = (button.text or "").lower()
                if any(w in btn_text for w in ["verify", "check", "проверить", "✅", "подписк"]):
                    log.info("[pasha] Clicking verify button: %s", button.text)
                    try:
                        await msg.click(button=button)
                        log.info("[pasha] Verify clicked, waiting for password prompt...")
                    except Exception as e:
    ...
    if "password" in text_low and ("enter" in text_low or "protected" in text_low):
        if latest_password:
            log.info("[pasha] Sending password EXACTLY as: [%s]", latest_password)
            await client.send_message(pasha_entity, latest_password)
        else:
            log.warning("[pasha] Bot asked for password but we don't have one stored yet!")
        return

    # ── Step 3: Log the final outcome ──
    if "incorrect" in text_low or "wrong" in text_low:
        log.warning("[pasha] ❌ Wrong password: %s", latest_password)
    elif any(w in text_low for w in ["success", "received", "получил", "gift", "подарок"]):
        log.info("[pasha] ✅ Gift claimed successfully!")
    elif "already" in text_low and "claimed" in text_low:
...
async def main():
    ...
    log.info("Watching @%s for Gift Cards via @%s ...", GIFT_CHANNEL, PASHA_BOT)
