...
async def fire_pasha_claim(start_param: str):
    ...
    global pasha_state, latest_password, bot_waiting_for_password
    pasha_state = "joining"
    latest_password = None
    bot_waiting_for_password = False
    ...
    try:
        ...
        log.info("[pasha] /start sent, waiting for subscription check or password...")
    except FloodWaitError as e:
...
async def on_password_reveal(event):
    ...
    global latest_password, bot_waiting_for_password, pasha_state
    latest_password = password
    log.info("[pasha] *** PASSWORD STORED (exact): [%s] ***", latest_password)

    # If bot is already waiting for password, send it right now
    if bot_waiting_for_password or pasha_state == "password_wait":
        log.info("[pasha] Sending password instantly to waiting bot: [%s]", latest_password)
        bot_waiting_for_password = False
        pasha_state = "idle"
        try:
...
async def on_pasha_bot_dm(event):
    """Handle @PashaGiftsBot DMs using a state machine to prevent loops."""
    if pasha_entity is None:
        return
    if event.sender_id != pasha_entity.id:
        return

    global pasha_state, bot_waiting_for_password

    msg = event.message
    text = msg.text or ""
    text_low = text.lower()

    log.info("[pasha] Bot said (state=%s): %s", pasha_state, text[:200])

    # ── STATE: joining — handle subscription check ──
    if pasha_state == "joining":
        if msg.buttons:
            # Join all URL channel buttons — log EVERY button's URL so we can debug
            for row in msg.buttons:
                for button in row:
                    url = getattr(button, "url", None)
                    btn_text = button.text or ""
                    log.info("[pasha] Button: [%s] url=%s", btn_text, url)

                    if not url or "t.me/" not in url.lower():
                        continue

                    # Skip verify button
                    if any(w in btn_text.lower() for w in ["проверить", "verify", "✅"]):
                        continue

                    # Handle private invite links: t.me/+HASH
                    invite_match = re.search(r"t\.me/\+([A-Za-z0-9_-]+)", url)
                    # Handle public username links: t.me/username
                    ...
                    if invite_match:
                        ...
                        try:
                            ...
                            log.info("[pasha] Joining via invite hash: %s ...", invite_hash)
                            await client(ImportChatInviteRequest(invite_hash))
                            log.info("[pasha] Joined via invite ✅")
                        except Exception as e:
                            log.warning("[pasha] Could not join via invite %s: %s", invite_hash, e)
                    elif username_match:
                        username = username_match.group(1)
                        if username.lower() not in (PASHA_BOT.lower(), "start", "share"):
                            try:
            ...
            # Find and click verify button using index (works for callback buttons)
            for i, row in enumerate(msg.buttons):
                for j, button in enumerate(row):
                    btn_text = (button.text or "").lower()
                    if any(w in btn_text for w in [
                        "проверить", "verify", "check", "✅", "подтвердить"
                    ]):
                        try:
                            log.info("[pasha] Clicking verify: [%s]", button.text)
                            await msg.click(i, j)
                            log.info("[pasha] Verify clicked ✅ — now waiting for password prompt")
                            pasha_state = "password_wait"
                        except Exception as e:
                            log.warning("[pasha] Verify click failed: %s", e)
                        return
        return  # still in joining state, wait for next bot message

    # ── STATE: password_wait — only handle password prompt ──
    if pasha_state == "password_wait":
        # Bot explicitly asks for password
        if "пароль" in text_low or "введите" in text_low or \
           ("password" in text_low and "enter" in text_low):
            if latest_password:
                log.info("[pasha] Sending password: [%s]", latest_password)
                await client.send_message(pasha_entity, latest_password)
                pasha_state = "idle"
                bot_waiting_for_password = False
            else:
                log.info("[pasha] Bot waiting for password from channel...")
                bot_waiting_for_password = True
            return

        # Log outcomes
        if "неверн" in text_low or "incorrect" in text_low:
            log.warning("[pasha] ❌ Wrong password: [%s]", latest_password)
            pasha_state = "idle"
        elif any(w in text_low for w in ["получил", "успешно", "вручён", "success"]):
            log.info("[pasha] ✅ Gift claimed!")
            pasha_state = "idle"
        # ignore promo/shop messages silently
        return

    # ── STATE: idle — ignore everything ──
    log.info("[pasha] Ignoring bot message (state=idle)")
